...
export function offerSlots(
  ...
  const opening =
    cancha.local === 1 ? config.apertura_sabana : config.apertura_guada;
  const closing =
    cancha.local === 1 ? config.cierre_sabana : config.cierre_guada;
  ...
  const at = (minute: number) =>
    `${addDays(date, Math.floor(minute / 1440))} ${String(Math.floor((minute % 1440) / 60)).padStart(2, "0")}:${String(minute % 60).padStart(2, "0")}:00`;
  const result: OfferSlot[] = [];
  for (let start = first; start + 60 <= last; start += 60)
    result.push({ start: at(start), end: at(start + 60) });
  return result;
}
export function slotIsAvailable(
  slot: OfferSlot,
  reservations: Pick<OfferReservation, "hora_inicio" | "hora_fin">[],
  now = new Date(),
): boolean {
  ...
  return (
    ...
    !reservations.some(
      (r) =>
        offerInstant(r.hora_inicio).getTime() < end &&
        offerInstant(r.hora_fin).getTime() > start,
    )
  );
}
export function eligibleOfferClients(
  history: OfferReservation[],
  week: Pick<OfferReservation, "celular_reserva">[],
  slot: OfferSlot,
  sedeCanchaIds: number[],
  now = new Date(),
): OfferClient[] {
  const target = costaRicaNow(offerInstant(slot.start));
  const targetWeekday = new Date(
    `${target.slice(0, 10)}T12:00:00Z`,
  ).getUTCDay();
  const targetMinute =
    Number(target.slice(11, 13)) * 60 + Number(target.slice(14, 16));
  const booked = new Set(
    week.map((r) => normalizeOfferPhone(r.celular_reserva)).filter(Boolean),
  );
  ...
  for (const reservation of history) {
    ...
    if (
      !phone ||
      booked.has(phone) ||
      !sedeCanchaIds.includes(reservation.cancha_id) ||
      instant.getTime() < lower ||
    ...
    const priority =
      weekday === targetWeekday
        ? 0
        : weekday === (targetWeekday + 1) % 7
          ? 1
          : weekday === (targetWeekday + 6) % 7
            ? 2
            : -1;
    if (
      priority < 0 ||
      Math.abs(minute - targetMinute) > (priority === 0 ? 180 : 120)
    )
      continue;
    ...
    if (
      ...
      (priority === previous.priority &&
        instant > offerInstant(previous.client.lastReservation))
    )
      clients.set(phone, {
        priority,
        client: {
          phone,
          name: reservation.nombre_reserva?.trim() || "Cliente",
          lastReservation: reservation.hora_inicio,
          lastCanchaId: reservation.cancha_id,
        },
      });
  }
  return [...clients.values()]
    .sort(
      (a, b) =>
        ...
        offerInstant(b.client.lastReservation).getTime() -
          offerInstant(a.client.lastReservation).getTime() ||
        a.client.phone.localeCompare(b.client.phone),
    )
    .map((entry) => entry.client);
}
export function offerDateLabel(value: string): string {
  return offerInstant(value).toLocaleDateString("es-CR", {
...
export function offerTimeLabel(value: string): string {
  return offerInstant(value).toLocaleTimeString("es-CR", {
    timeZone: "America/Costa_Rica",
    hour: "numeric",
    minute: "2-digit",
    hour12: true,
